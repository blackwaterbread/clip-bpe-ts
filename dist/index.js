...
const bpe_simple_vocab_16e6_1 = __importDefault(require("./bpe_simple_vocab_16e6"));
function ord(c) {
...
function bytesToUnicode() {
    ...
    let cs = bs.slice(0).map(x => x.toString());
    let n = 0;
...
function basicClean(text) {
    ...
    text = (0, html_entities_1.decode)((0, html_entities_1.decode)(text));
    return text.trim();
}
...
class Tokenizer {
    ...
    constructor() {
        ...
        let merges = bpe_simple_vocab_16e6_1.default.text.split("\n");
        merges = merges.slice(1, 49152 - 256 - 2 + 1);
        let mergesPair = merges.map(merge => merge.split(" "));
